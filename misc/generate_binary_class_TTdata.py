...
def merge_csvs(category_data, all_info_csv_path ,header_csv, output_folder):
    ...
    for category, data in category_data.items():
        ...
        length = len(csv_paths)
        # split the csv_paths into two parts : test(20%) and train(80%)
        test_csv_paths = csv_paths[:int(length*0.2)]
        train_csv_paths = csv_paths[int(length*0.2):]
        combine_csvs(category, test_csv_paths, all_info_table, header_csv, os.path.join(output_folder, "test"))
        combine_csvs(category, train_csv_paths, all_info_table, header_csv, os.path.join(output_folder, "train"))
        
def main(root_folder, output_folder):
    category_data = find_videos_and_csvs(root_folder)
    merge_csvs(category_data, "/home/mcnlab/桌面/VitPose/ViTPose/poses_videos_out_train/info.csv", "poses_videos_out_train/data.csv", output_folder)
